...
def main():
    ...
    outputs = [
        ...
        # common.TensorBoardOutput(logdir),  # 输出的文件占用存储空间太大
    ]
    ...
    mae_train_dataset = iter(train_replay.dataset(**config.mae_dataset))  # mae
    report_dataset = iter(train_replay.dataset(**config.dataset))
    agnt = agent.Agent(config, obs_space, act_space, step)
    train_mae = agnt.train_mae  # mae
    train_agent = common.CarryOverState(agnt.train)

    train_mae(next(mae_train_dataset))  # mae
